...
class MyDataset(Dataset):
    def __init__(self, path, mode):
        ...
        for file_name in os.listdir(path):
            if file_name.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif', '.tiff', '.tif')):
                self.img_path.append(file_name)

        if mode == "train":
            self.transform = transforms.Compose([
                transforms.Resize((350, 350)),  # 299
                transforms.ToTensor(),
    ...
    def __getitem__(self, i):
        ...
        return self.img_path[i], image  # , label
...
def main(batch_size, img_file_path):
    device = "cuda:0" if torch.cuda.is_available else "cpu"

    dataset = MyDataset(img_file_path, "test")
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=6)

    model = torch.load("checkpoint/model_baseline2_test_98.35897435897436.pt", map_location=device).to(device)
    model.eval()
    ...
    df.to_csv('ffdi_predict.csv', index=False)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Inference Script')
    parser.add_argument('--batch_size', type=int, default=8, help='Batch size for inference')
    parser.add_argument('--img_file_path', type=str, required=True, help='Path to the img file')

    args = parser.parse_args()

    main(args.batch_size, args.img_file_path)
